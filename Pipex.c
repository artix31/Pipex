...
/*   Updated: 2025/01/28 17:12:15 by amashhad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_cmd2_operation(char *argv[], char *envp[], int pipe_fd[])
{
	...
	{
		close_fds(pipe_fd, 2);
		ft_errmsg("File Creation Failed", outfile);
	}
...
int	main(int argc, char *argv[], char *envp[])
{
	...
	if (pipe(pipe_fd) == -1)
		ft_errmsg("Pipe Failed", -1);
	pid[0] = fork();
	if (pid[0] == -1)
		ft_errmsg("Fork Failed", -1);
	if (pid[0] == 0)
